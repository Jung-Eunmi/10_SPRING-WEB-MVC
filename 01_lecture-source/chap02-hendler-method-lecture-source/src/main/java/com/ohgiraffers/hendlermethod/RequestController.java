...
public class RequestController {
    ...
    *       -> key 와 value 형태로 값이 전달 되고있다.
    *      나중에 나올 개념이 JSON(자바스크립트객체표현식) 으로 전달이 되면
    *      Jackson 컨버터 (자바스크립트 객체 <--> 자바 객체) 로
    *      자동 변환해주어 프론트엔드 서버(js 기반) 백엔드 서버(Java 기반) 간의
    *      데이터 전송을 할 수 있게 해준다.
    ...
    *      일반적인 form 태그에서는 거의 사용하지 않는다. */
    ...
    public void bodyTest(@RequestBody String body) throws UnsupportedEncodingException {
        ...
        /* comment.
        *   넘어 온 값을 보면 알 수 없는 값으로 변환되어 있다.
        *   이 것을 encoding(암호화) 되어있다 라고 말하며
        *   해석하기 위해서는 decoding(해석) 을 해야한다. => URLDecoder 사용 */

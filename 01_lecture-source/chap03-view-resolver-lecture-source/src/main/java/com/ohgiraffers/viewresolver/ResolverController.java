...
public class ResolverController {
    ...
    *   뷰 리졸버는 타임리프 문법을 해석할 수 있는 ThymeleafViewResolver 로 전환되며
    *   이는 prefix 로 resource/templates/suffix 로 .html 을 자동으로 붙여주게 된다. */
    @GetMapping("string")
    ...
    /* comment.
     *   redirect 란 최초 요청을 받게 되면 응답을 진행 후 다시 재요청 하는 방식이다.
     *   따라서 request 가 2번 일어나기 때문에 값을 유지 할 방법이 없다.
     *   그래서 Servlet 에서 Session, Cookie 라는 개념을 했었다.
     *   Session(server 에 공간 생성) 과 Cookie(클라이언트에 공간 생성) 는
     *   별도의 공간을 할당하기때문에 관리 할 데이터가 많아지면 서버의 부담이 생길 수 있다.
    ...
    public String redirectAttr(RedirectAttributes attr) {
        ...
        *   Flash(잠깐 값이 담결다가 소멸되는 방식) 를 사용한다.
        *   Session 처럼 소멸 될 때까지 공간을 차지하는 것이 아닌
        *   잠시 데이터를 썼다가 사라지는 구조이기 때문에 서버에 부담이 없다. */
    ...
    *   값을 집어넣을 수도 있고, 화면을 결정 지을 수도 있다. */
    ...
    public ModelAndView mvRedirectAttr(ModelAndView mv, RedirectAttributes attr) {
        ...
        attr.addFlashAttribute("flashMessage2","ModelAndView redirect 시 값 유지");
